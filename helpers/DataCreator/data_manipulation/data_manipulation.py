...
from data_manipulation.helpers import combine_similar_dfs, add_meta_info_df, check_df_bad_read, try_read_csv


def handle_date_and_dollars(self, df:pd.DataFrame, dollar_to_dop:float) -> pd.DataFrame:
  """
...
def combine_savings_df(self) -> pd.DataFrame:

  df = combine_similar_dfs(self.df_list, 'Ahorros')
  return df


def combine_credit_cards_df(self) -> pd.DataFrame:

  df = combine_similar_dfs(self.df_list, 'Tarjeta de Credito')
  return df
